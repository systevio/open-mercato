...
Open **Sales → Configuration**. The only provider the platform ships is the built-in default:

| Provider | What it does |
|---|---|
| **Default (Tax rates)** | Uses the tax rates configured in the **Tax rates** section, exactly as the calculation engine already did. Selecting nothing is the same as selecting this. |

Install a tax provider package — Avalara, for example — and a **Tax provider** section appears above the tax section with a select listing the default alongside it. Until then there is nothing to choose between, so the page shows the **Tax rates** table alone.

Selecting an external provider replaces the **Tax rates** table with that provider's own section: its options, the **ship-from address** an engine needs to determine origin-based tax, and a **timeout** (1000–30000 ms) that bounds how long a provider may delay a document write. The default provider needs none of those — it reads the rates you already maintain — so they are not shown for it.

:::info Nothing changes until you choose
The default provider is an exact identity over the existing calculation engine. An organization that never opens this page keeps every amount it has today, to the cent.
:::
