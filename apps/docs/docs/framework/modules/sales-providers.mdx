...
|---|---|---|
| Selected on | the method row | the organization (`Sales → Configuration → Tax provider`) |
...
The built-in `table-rates` provider is an exact identity over the calculation engine, so an organization that selects nothing keeps every amount it had before this slot existed.
