...
:::note Unit amounts vs. whole documents
Everything on this page overrides how **one amount** is turned into its net/gross pair. It cannot see the addresses, the customer or the sibling lines, so it cannot implement a jurisdiction-aware engine.

For that, register a **document level tax provider**: see [Building a tax provider](./modules/building-tax-provider) and [Shipping, payment & tax providers](./modules/sales-providers). A provider receives the whole document and is selected per organization.
:::
